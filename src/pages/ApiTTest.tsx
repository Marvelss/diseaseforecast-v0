...
const ApiTTest: React.FC = () => {
  ...
  const handleSelectAll = (e: any) => {
    ...
    form.setFieldsValue({ comparedVariables: e.target.checked ? features : [] });
  };

  const handleFinish = async (values: any) => {
    setLoading(true);
    try {
      const res = await apiRequest<{ data: any }>('/feature-optimization-dataset/t-test', {
        method: 'POST',
        body: new URLSearchParams({
          targetVariable: values.targetVariable,
          comparedVariables: values.comparedVariables.join(','),
  ...
  const comparedVariables = Form.useWatch('comparedVariables', form) || [];
  ...
  return (
    <div style={{ padding: 32, textAlign: 'left', maxWidth: 900, margin: '0 auto' }}>
      ...
      <Form form={form} layout="vertical" onFinish={handleFinish} style={{ marginBottom: 24 }}>
        ...
        <Form.Item name="comparedVariables" label="对比特征变量" rules={[{ required: true, message: '请选择特征变量' }]}> <Select mode="multiple" options={features.map(f => ({ label: f, value: f }))} /> </Form.Item>
        ...
        <Button type="primary" htmlType="submit" loading={loading}>开始检验</Button>
      </Form>
      <div style={{ marginBottom: 16 }}>
        {comparedVariables.map((f: string) => (
      ...
      {result && (
        <Table
          ...
          columns={[{ title: '特征', dataIndex: 'key' }, { title: '结果', dataIndex: 'value' }]}
          pagination={false}
        />
      )}
